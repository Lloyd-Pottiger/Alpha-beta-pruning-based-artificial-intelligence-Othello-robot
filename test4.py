...
class AI(object):
    ...
    def judge(self, color, chessboard):
        ...
        weight = self.change_weight(chessboard, weight)
        ...
        if my_chess > opp_chess:
            c = ((100 * my_chess) // (my_chess + opp_chess)) * 5
        elif my_chess < opp_chess:
            c = -((100 * opp_chess) // (my_chess + opp_chess)) * 5
    ...
    def change_weight(self, chessboard, weight):
        size = self.chessboard_size
        if chessboard[0][0] != COLOR_NONE:
            weight[0][1] = 8
            weight[1][0] = 8
        ...
        if chessboard[size - 1][0] != COLOR_NONE:
            ...
            weight[size - 2][1] = 11
        if chessboard[size - 1][size - 1] != COLOR_NONE:
            weight[size -1][size -2] = 8
            weight[size -2][size -1] = 8
            weight[size -2][size -2] = 11
        return weight
