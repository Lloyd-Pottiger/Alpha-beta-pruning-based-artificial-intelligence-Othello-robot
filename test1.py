...
class AI(object):
    ...
    def alpha_beta(self, alpha, beta, depth, color, chessboard, p=0):
        ...
        for (i, j) in list:
            ...
            value = 0
            if depth <= 1:
                value = -self.judge(-color, chessboard)
            else:
                value = -self.alpha_beta(-beta, -alpha, depth - 1, -color, chessboard)[0]
    ...
    def judge(self, color, chessboard):
        ...
        count = 2 * np.sum(chessboard * weight)
        my_chess = 0
        opp_chess = 0
        my_tiles = 0
        opp_tiles = 0
        ...
        s = (self.get_stable(color, chessboard) -
             self.get_stable(-color, chessboard)) * 200

        my_mobility = len(self.get_valid_point(color, chessboard, False))
        opp_mobility = len(self.get_valid_point(-color, chessboard, False))
        m = 0
    ...
    def place(self, x, y, color, chessboard):
        ...
        for d in range(8):
            i = x + DIRECTION[d][0]
            j = y + DIRECTION[d][1]
            while 0 <= i < self.chessboard_size and 0 <= j < self.chessboard_size and chessboard[
                    i][j] == -color:
                i += DIRECTION[d][0]
                j += DIRECTION[d][1]
            if 0 <= i < self.chessboard_size and 0 <= j < self.chessboard_size and chessboard[
                    i][j] == color:
                while True:
                    i -= DIRECTION[d][0]
                    j -= DIRECTION[d][1]
    ...
    def get_valid_point(self, color, chessboard, sort = True, reverse = False):
        ...
        for (m, n) in idx:
            for d in range(8):
                ...
                j = n + DIRECTION[d][1]
                time = 0
                while 0 <= i < self.chessboard_size and 0 <= j < self.chessboard_size and \
                    chessboard[i][j] == -color:
                    i += DIRECTION[d][0]
                    j += DIRECTION[d][1]
                    time += 1
                if 0 <= i < self.chessboard_size and 0 <= j < self.chessboard_size and \
                    chessboard[i][j] == color and time > 0:
